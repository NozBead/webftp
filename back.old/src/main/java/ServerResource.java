...
public class ServerResource {
	...
	public Response addServer(	@FormParam("alias") String alias,
								...
								@DefaultValue("true")
								@FormParam("passive") boolean passive) {
		
		URI uri = getFtpUrl(urlStr);
		if (uri == null) {
			return Response.status(Status.BAD_REQUEST).entity("Bad Ftp URL.").build();
		}
		...
		URI createdUri = contextUri.getRequestUri().resolve(alias);
